...
router.get('/api/v1/orders', function(req, res, next) {
	// console.log(ida.odf_data);
	// // res.render('index', { title: ida.odf_data });
	// res.setHeader('Content-Type', 'application/json');
	// res.json({data: ida.odf_data});
	...
	db.query(sql, function(err, rows) {
		...
		for(var i = 0; i < order_list.length; i++) {
			...
			for(var j = 0; j < item_list.length; j++) {
				...
				// let item_name = temp[0];
				// let item_num = temp[1];
				// let item_image = temp[2];
				let itemList_json = {
					id: temp[0],
					name: temp[1],
					price: temp[2],
	...
	db.query(sql, [order_uid], function(err, rows) {
		...
		if(rows[0]["driverID"]) {
			...
			for(var k = 0; k < data_temp.length; k++) {
				...
				if(data_temp[k]["driverId"] == rows[0]["driverID"]) {
					driverAddress_json["lat"] = data_temp[k]["lat"]
					driverAddress_json["lng"] = data_temp[k]["long"]
					console.log(driverAddress_json);
					k = 100;
				}
			}
