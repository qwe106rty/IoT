...
class ConnThread6 constructor(val activity: Activity, val lv: ListView, val token: String?,val driveId:Int?): Thread() {
...
//                Toast.LENGTH_SHORT).show() }
        try {
            ...
            val urlstr= "http://140.113.123.90:9527/appGetDriverOrders?token="+token+"&driverID="+driveId.toString()
            Log.d("OAO",urlstr)
            val url = URL(urlstr)
    ...
    fun connsucc(al: ArrayList<Order>) {
        activity.runOnUiThread {
            val adapter = MyAdapter2(activity, al ,token,driveId )
            lv.adapter = adapter
